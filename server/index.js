...
app.get('/items', function (req, res) {
  ...
  var restaurants  = ["Elixiria", "Hive Coffee Bar", "Hanlin Tea House"];
  ...
  restaurants.map((restaurantName) => {
    ...
    .then(response => {
      ...
      restaurantsWithRating.push(restaurant);
      if(restaurantsWithRating.length === 3){
        res.end(JSON.stringify(restaurantsWithRating));
      }
    })
    .catch(err => {
      console.log(err);
...
const fetchRestaurant = function(url, otherPram){
  fetch(url, otherPram)
    .then(data => { return data.json(); })
    .then(res => {
      console.log(res);
